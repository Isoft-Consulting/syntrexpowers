...
- real enforcing activation requires an explicit blocking-mode acknowledgement flag after plan review, while `--enforce --plan-only` remains available without that acknowledgement and must not mutate install roots or provider configs
