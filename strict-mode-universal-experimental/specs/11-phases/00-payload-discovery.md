...
- imported typed `command-execution`, `matcher`, and `event-order` records must not be self-attested: command-execution requires typed proof plus matching discovery-record/stdout/stderr/exit-code fixtures, matcher requires typed proof plus a matching discovery-record fixture, event-order requires typed proof plus discovery-record fixtures for the observed order, and all referenced payload hashes must match checked-in raw payload fixtures for the same provider/event
