...
- `verdict` is one of `clean`, `challenge`, or `unknown`. `findings` is an empty array for `clean` and `unknown`; for `challenge`, every finding contains exactly `severity`, `source`, `path`, `harm`, and `fix`. Judge finding `severity` is `critical`, `high`, `medium`, `low`, or `info`; `source` is `artifact`, `assistant-text`, or `scope-metadata`; `path` is a normalized project-relative path from the reviewed scope or an empty string for cross-file findings; `harm` and `fix` are non-empty UTF-8 strings capped at 4096 bytes each after redaction. `reviewed_scope_digest` and `reviewed_artifact_hash` must equal the current FDR challenge inputs. `confidence` is a finite canonical JSON decimal from `0` to `1`, with no exponent and at most three fractional digits. `response_hash` is SHA-256 over canonical response JSON with only `response_hash` set to an empty string.
