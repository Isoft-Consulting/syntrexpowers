...
- `stop-guard.sh` — extended с 4 новыми проверками поверх Wave 2 stub-scan: (a) edits + no artifact + non-trivial → block "FDR artifact missing"; (b) artifact present → run validator → propagate findings; (c) artifact + open findings + edits-mtime > artifact-mtime → block recheck; (d) sensitive-paths → run fdr-verify (Phase 5 hook, no-op без verifier). Все 4 idempotent через `[[ -x ]]` guards.
