...
**Docs ТРЕБУЮТ ФДР** (`*.md`, `*.rst`, `*.txt`, README, CHANGELOG, спеки) — изменения в документации могут вносить incorrectness в инструкции, противоречия между разделами, неточные claims о поведении системы (number-discrepancies, файловые пути, версии).

- **В Wave 2.5 (текущее)** docs-edits триггерят `missing-verdict` через `fdr-challenge.sh` (если есть FDR-context в transcript) — агент обязан дать verdict. Skip возможен только через `~/.claude/state/bypass-<sid>` или per-project disable.
- **В Wave 3 (design only)** runner будет парсить per-commit opt-in: `git commit --trailer 'Strict-Skip-FDR: docs trivial typo fix'` → пропуск с записью в `bypass-log.jsonl`. Реализация в Phase 2 (когда `is-trivial-diff.sh` появится).

Если хоть один файл вне skip-list — ФДР обязателен.

**Precedence: sensitive-paths overrides skip-list.** Если файл совпадает И со skip-list (например, `*.lock`), И с sensitive-paths (например, `auth/*`) — ФДР запускается полностью, включая independent verifier.
...
### Wave 3 Phase 2 — Core enforcement (deployed 2026-05-04 to staging)

Bundle ships в `~/strict-mode-bundle/hooks/`, ещё НЕ deploy'ено в `~/.claude/hooks/` (требует `bash install.sh` на target).

**Components:**
- `is-trivial-diff.sh` — skip-list определитель. Tracked + untracked файлы (через `git ls-files --others --exclude-standard`). Проверки: whitespace-only, comments-only (PHP/Go/JS/TS/Python/Ruby/sh), lockfiles (composer/package/cargo/poetry/etc), translations (lang/i18n/locales). Sensitive-paths overrides skip. **Docs убраны из skip-list** (по правилу 2026-05-04, см. E9). Docs триггерят non-trivial → ФДР обязателен.
- `fdr-validate.sh` — артефакт валидатор. Реализовано: C1 (artifact exists), C2 (scope completeness), C3 (7 required fields per finding + CRITICAL/HIGH need :line), C4 (severity vocab), C5 (status vocab), C6 (verdict 2-line format + open-vs-complete consistency), C7 (cycles ≥ 2 после resolved), C8 (forbidden sections + praise phrases regex), C9 (Layer 0 if Phase 4 active), C10 (resolved → fix-commit + re-check), C11 (bypass file).
- `stop-guard.sh` — extended с 4 новыми проверками поверх Wave 2 stub-scan: (a) edits + no artifact + non-trivial → block "FDR artifact missing"; (b) artifact present → run validator → propagate findings; (c) artifact + open findings + edits-mtime > artifact-mtime → block recheck; (d) sensitive-paths → run fdr-verify (Phase 5 hook, no-op без verifier). Все 4 idempotent через `[[ -x ]]` guards. **Transitional flag `STRICT_NO_ARTIFACT_GATE=1` отключает (a) и (c)** — критично пока Phase 3 (`/fdr` skill) не deploy'ен, иначе нет способа генерировать артефакт и каждый Stop с edits заблокируется.
- `install.sh` — обновлён, копирует Wave 3 hooks отдельным WAVE3_HOOKS array.

**Tests:** 200/200 (51 новых для Phase 2: W4.1-W4.21 для is-trivial-diff + fdr-validate, W5.1-W5.8 для расширенного stop-guard).

**Bugs caught & fixed during Phase 2 development:**
- `IFS='|||'` в bash — это set из 3 одинаковых символов, не multi-char delimiter. Findings parser переписан через awk → temp-files per finding.
- `git diff` пропускает untracked файлы → новые файлы выглядели как «нет изменений = trivial». Fix: добавил `git ls-files --others --exclude-standard` для untracked detection.
