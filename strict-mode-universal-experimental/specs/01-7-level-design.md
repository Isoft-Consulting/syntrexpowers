...
- The generated hook command passes `STRICT_HOOK_TIMEOUT_MS=<milliseconds>` and `STRICT_STATE_ROOT="<state-root>"` as literal environment assignments before the quoted `<install-root>/active/bin/strict-hook` path. Enforcing blocking commands also pass `STRICT_ENFORCING_HOOK=1` and `STRICT_OUTPUT_CONTRACT_ID="<selected-output-contract-id>"` so unreadable install state can still fail closed instead of silently downgrading to discovery. These assignments are trusted only because they are part of protected generated hook config and match the protected install baseline; provider tool environment cannot override them.
