...
Status: draft documentation package with executable metadata scaffolding, provider proof fixtures, conservative payload normalization, discovery runtime installation, readiness-gated enforcing installation, and fixture-bound provider block output. The checked-in Claude/Codex fixture manifests are still empty, so real enforcing activation remains blocked in this directory until provider fixture sets are captured and validated.
