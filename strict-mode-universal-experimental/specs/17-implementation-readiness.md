...
| `install.manifest.v1` | `lexical active strict-hook command binding` | managed hook entry | `STRICT_HOOK_TIMEOUT_MS=<ms> STRICT_STATE_ROOT="<state-root>" "<install-root>/active/bin/strict-hook" --provider <provider> <logical_event>`; timeout/state-root/provider/event parity | release realpath; outside install root; shell wrapper; extra argv; provider drift |
