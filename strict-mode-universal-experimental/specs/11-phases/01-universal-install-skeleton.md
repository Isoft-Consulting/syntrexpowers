...
- legacy active runtime directories are migrated only when previous manifest/baseline fingerprints prove they are strict-mode-owned; unrecognized directories at `<install-root>/active` make activation fail before provider configs are touched
