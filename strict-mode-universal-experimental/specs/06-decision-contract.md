...
Protected judge prompt templates are inputs to semantic judge prompt construction only. Template text, template hashes, template diagnostics, and template-derived JSON examples must not add fields to `judge.response.v1`, must not alter the closed judge reason enum, and must not be emitted as a substitute for canonical provider output. The deterministic local `strict-judge` classifier is also bound to the same closed response object; its internal classification labels must be mapped to `verdict`, `reason`, and `findings` before provider output emission.
...
- Stop block must use the Codex-recognized stop hook continuation format. For Codex `0.128.0`, `decision:"block"` on Stop does not hard-deny the final answer; it continues the same turn with the supplied reason as an additional prompt, and the hook must allow the follow-up Stop when `stop_hook_active=true` to avoid recursion. The same recursion-guard rule applies to Claude `Stop` and `SubagentStop` whenever the captured payload exposes `stop_hook_active=true`: strict-hook returns `allow` for the follow-up event without further preflight inspection. The guard is gated on `provider_detection_decision="match"` and a non-truncated payload, so a forged or partial payload cannot bypass blocking. Fixture test required before enabling fail-closed.
