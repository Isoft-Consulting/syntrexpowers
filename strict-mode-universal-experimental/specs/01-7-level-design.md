...
- If the self-deadline fires before a required trusted-state transaction commits and re-verifies its ledger coverage, that transaction is treated as uncommitted. Enforcing `pre-tool-use`, `permission-request`, and `stop` emit the fixture-verified block/deny decision for `hook-self-timeout` rather than allow. `post-tool-use`, prompt, and health events may return the safest provider-compatible allow/no-op only after leaving no trusted partial record behind; later Stop must treat the missing tool/edit evidence as unresolved or untrusted rather than silently accepting it.
